...
def send_mail(message, addresses, subject, immediate=True):
    ...
    for address in addresses:
        ...
        except (socket.error, SMTPException, MailHostError) as e:
            logger.warn('Could not send email to %s with subject %s %s',
                        address, subject, e)
        except:
            raise
...
def create_plaintext_message(message):
    ...
    parser.close()
    # append the anchor list at the bottom of a message
